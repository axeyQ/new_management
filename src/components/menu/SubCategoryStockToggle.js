// src/components/menu/SubCategoryStockToggle.js
'use client';
...
const SubCategoryStockToggle = ({ subCategory, onSuccess }) => {
  ...
  const getOutOfStockModeCount = () => {
    if (!subCategory?.stockStatus?.orderModes) return 0;
  ...
  // Determine if subCategory has mixed status (some modes out of stock, others in stock)
  ...
  const handleSuccessCallback = (updatedSubCategory) => {
    if (onSuccess) {
        ...
        <Box sx={{ p: 2, maxWidth: 320 }}>
          ...
          <Box>
            {ORDER_MODES.map(mode => {
              const modeOutOfStock = subCategory?.stockStatus?.orderModes?.[mode.value]?.isOutOfStock;
...
export default SubCategoryStockToggle;